...
export const bladesRollModifierList = {
    ...
    resolveFunc: (fields, extraData) => {
      ...
      if (costType == 'Stress') {
        if (!fields['SFTD.NotAProblem']) {
          otherStress[assistFull.id] = 1;
          stressCost = 1;
        } else
          otherValue[assistFull.id] = {'system.not_a_problem_uses.value': -1};
      }
      return {
        dice: 1,
  ...
  resilience: {
    name: 'SFTD.CrewAbility.Resilience.Title',
    rollType: 'resistance',
    dice: 1
  },
  scrapyards_construct_foundation: {
    rollType: 'constructFoundation',
    ticks: 1
  },
  scrapyards_long_term_project: {
    name: 'SFTD.CrewAbility.Scrapyards.LongTermProjectTitle',
    rollType: 'longTermProject',
    dice: 1
  },
  sum_of_its_parts: {
    name: 'SFTD.CrewAbility.SumOfItsParts.Title',
    rollTypes: ['actionRoll', 'groupAction'],
    needHarmony: true,
    rollText: 'SFTD.CrewAbility.SumOfItsParts.Description',
  },
  disaster_relief: {
    name: 'SFTD.CrewAbility.DisasterRelief.Title',
    rollType: 'longTermProject',
    dice: 1
  },
  forged_in_fire: {
    name: 'SFTD.CrewAbility.ForgedInFire.Title',
    rollType: 'resistance',
    dice: 1
  },
  reassuring_action_1: {
...
export async function bladesRoll(diceAmount, attributeOrRollName = '', note = '', extraFields = {}) {
  ...
  let stressChanges = {};
  stressChanges[extraFields.actor?.id] = 0;
  ...
  for (let modifier of extraFields.modifiers) {
    ...
    if (modifier.stress) stressChanges[extraFields.actor?.id] += Number(modifier.stress);
    ...
    if (modifier.otherValue)
      for (let [id, value] of Object.entries(modifier.otherValue))
  ...
  for (let [stressActorId, stressChange] of Object.entries(stressChanges)) {
    let stressChangeItem = {value: stressChange, realValue: stressChange};
    let stressActorFull = BladesHelpers.resolveActor(`Actor.${stressActorId}`);
    if (stressChange != 0 && stressActorFull?.system.stress?.value != undefined) {
      let resultStress = Math.clamp(stressActorFull.system.stress.value + stressChange, 0, stressActorFull.system.stress.max);
      stressChangeItem.realValue = resultStress - stressActorFull.system.stress.value;
      ...
      rollData.stressChanges[stressActorId] = stressChangeItem;
    }
  }
  ...
  for (let [otherActorId, otherChangeObj] of Object.entries(otherChanges)) {
    if (Object.values(otherChangeObj).length == 0) continue;
    let otherActorFull = BladesHelpers.resolveActor(`Actor.${otherActorId}`);
    if (otherActorFull) {
      ...
      rollData.otherChanges[otherActorId] = otherChangeItem;
    }
  }
...
async function showChatRollMessage(r, zeromode, attributeOrRollName, note, extraFields) {
  ...
  // Check for Resistance roll
    ...
    if (!extraFields.rollData.charmwork) {
      let rollStressValue = (extraFields.rollData.stressChanges[extraFields.actor.id]?.value ?? 0) + stress;
      ...
      extraFields.rollData.stressChanges[extraFields.actor.id] = {value: rollStressValue, realValue: newStress - Number(extraFields.actor.system.stress.value)};
...
export async function cancelRollResult(rollData, actorFull) {
  ...
  for (let [stressChangeId, stressChange] of Object.entries(rollData.stressChanges)) {
    ...
    await BladesHelpers.tryUpdate(stressActorFull, {'system.stress.==value': Math.clamp(stressActorFull.system.stress.value - stressChange.realValue, 0, stressActorFull.system.stress.max)});
  }
  ...
  for (let modifier of rollData.modifiers) {
    if (modifier.itemNeeded) {
      ...
      if (actorFull.items)
        exhaustableItems = exhaustableItems.concat(actorFull.items.filter(i => i.system[modifier.itemNeeded] && i.system.uses.value < i.system.uses.max));
      if (crewFull)
        exhaustableItems = exhaustableItems.concat(crewFull.items.filter(i => i.system[modifier.itemNeeded] && i.system.uses.value < i.system.uses.max));
...
export function pruneInvalidConditionalRollModifiers(actorFull, modifiers) {
  ...
  for (let modifier of modifiers) {
    ...
    if (modifier.itemNeeded) {
      ...
      let validItems = [];
      if (actorFull.items)
        validItems = validItems.concat(actorFull.items.filter(i => i.system[modifier.itemNeeded] && (i.system.uses.max == 0 || i.system.uses.value > 0)));
      if (crewFull)
        validItems = validItems.concat(crewFull.items.filter(i => i.system[modifier.itemNeeded] && (i.system.uses.max == 0 || i.system.uses.value > 0)));
...
export async function postRollProcessing(actorFull, extraFields) {
  ...
  for (let modifier of extraFields.modifiers) {
    if (modifier.itemNeeded) {
      ...
      let exhaustableItems = [];
      if (actorFull.items)
        exhaustableItems = exhaustableItems.concat(actorFull.items.filter(i => i.system[modifier.itemNeeded] && (i.system.uses.max == 0 || i.system.uses.value > 0)));
      if (crewFull)
        exhaustableItems = exhaustableItems.concat(crewFull.items.filter(i => i.system[modifier.itemNeeded] && (i.system.uses.max == 0 || i.system.uses.value > 0)));
      if (exhaustableItems.length > 0)
